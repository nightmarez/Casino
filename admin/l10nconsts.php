	...
	<script>
		$(document).ready(function() {
			$('#apply').click(function() {
				...
				$.get('/admin/doaddconst.php?title=' + title, function(result) {
					if (result.trim() == 'true') {
						alertify.success('<?php echo getL10n("Consts-ConstAdded"); ?>');
						location.href = '/admin/l10nconsts.php';
					} else {
			...
			$('.save-button').click(function() {
				...
				$.get('/admin/dosaveconst.php?id=' + id + '&title=' + title, function(result) {
					if (result.trim() == 'true') {
			...
			$('.delete-button').click(function() {
				...
				$.get('/admin/dodeleteconst.php?id=' + id, function(result) {
					if (result.trim() == 'true') {
