...
?>
	...
	<div class="panel panel-default">
		...
		<table class="table">
					...
					while (list($id, $const, $text) = $req->fetch(PDO::FETCH_NUM))
					{
						?>
							<tr>
								...
								<td>
									<a href="#" class="btn btn-primary save-button" role="button" id="save-button-<?php echo $id; ?>">Save</a>
								</td>
							</tr>
						<?php
					}
			...
			<tfoot>
				<tr>
					<td><a href="#" class="btn btn-primary" role="button" id="save-button-all">save All</a></td>
				</tr>
			</tfoot>
		</table>
	</div>
	...
	<script>
		$(document).ready(function() {
			$('.save-button').click(function() {
				...
				$.get('dosavetext.php?lang=<?php echo $langid; ?>&const=' + id + '&text=' + text, function(result) {
					if (result.trim() == 'true') {
						alertify.success('Value saved');
					} else {
			...
			$('#save-button-all').click(function() {
				...
				var doneFunc = function() {
					if (ok) {
						...
						alertify.error('Some errors happens');
					}
				};
